...
`--build-raw-from-search` converts structured search results into raw candidates before any filtering. `config/search_queries.json` is the authoritative required-query list; `audit_search_log.py`, `build-approved`, and the send gate all verify that every required query was recorded. `--build-approved` is intentionally slower because it audits required queries, candidate URL coverage, and, by default, touches outbound links before report generation. It prevents stale/deleted articles from entering approved data and rejects reachable pages whose title signals clearly do not match the collected item title. Use `--relaxed-search-coverage`, `--skip-url-health`, or `--skip-title-match` only for offline tests or temporary diagnostics. Title-match network failures are warnings rather than blockers; see `docs/TITLE_URL_MATCH_GATE.md`.
