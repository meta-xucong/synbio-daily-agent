...
`--build-raw-from-search` converts structured search results into raw candidates before any filtering. `--build-approved --strict-search-coverage` is intentionally slower because it audits search coverage and, by default, touches outbound links before report generation. It prevents stale/deleted articles from entering approved data and rejects reachable pages whose title signals clearly do not match the collected item title. Use `--skip-url-health` / `--skip-title-match` only for offline tests or temporary diagnostics. Title-match network failures are warnings rather than blockers; see `docs/TITLE_URL_MATCH_GATE.md`.
