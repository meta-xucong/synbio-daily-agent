...
- [ ] 已创建 `data/search_log_YYYY-MM-DD.json`，覆盖 `config/search_queries.json` 中 r1-r6 全部 required query、LLM 动态 required query、`llm_discovery` 和 `llm_gap_audit`，并保留结构化搜索结果（title/url/snippet/source/date）
...
| 缺少搜索日志或 source_round | 无法证明 r1-r6 基座检索和 LLM 高召回检索，旧内容可能补录混入 | **发送前 pre_check 强制阻断** |
...
**标准回复模板**：
> "正在执行完整流水线：r1-r6基座搜索 + LLM高召回搜索 → 保存JSON → 脚本处理（去重+过滤） → 生成报告 → 验证 → 发送邮件。预计需要X分钟，请稍候。"
